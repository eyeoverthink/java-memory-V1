...
try:
    from llm_integration import LLMProcessor, is_api_key_configured
    LLM_AVAILABLE = True
except ImportError:
    LLM_AVAILABLE = False
    print("Warning: LLM integration not available. Using fallback quantum processor.")
...
class QuantumChatProcessor:
    def __init__(self):
        ...
        if LLM_AVAILABLE and is_api_key_configured():
            self.llm_processor = LLMProcessor()
            self.use_llm = True
            print("LLM processor initialized for enhanced quantum responses")
        else:
            self.llm_processor = None
            self.use_llm = False
            print("Using standard quantum processor (LLM not available)")
        
    def process_query(self, query):
        """Process user query through quantum systems or LLM"""
        # Use LLM if available, otherwise use quantum systems
        if self.use_llm and self.llm_processor:
            try:
                return self.llm_processor.process_query(query)
            except Exception as e:
                print(f"LLM processing error: {e}")
                # Fall back to quantum systems if LLM fails
                pass
