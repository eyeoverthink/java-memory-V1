...
public class KnowledgeScraper {
    ...
    private static final int MAX_CHUNKS_PER_FILE = 5000;  // Process full PDFs
    private static final long MAX_FILE_SIZE_MB = 100;     // Skip files larger than 100MB
    private static final int MAX_TEXT_LENGTH = 5000000;   // Max 5M chars to process per file
    ...
    private void processFile(Path path) throws Exception {
        ...
        if (fileSize > MAX_FILE_SIZE_MB * 1024 * 1024) {
            CommandTerminal.printColored(String.format("  Skipping %s (%.1f MB - too large)", filename, fileSize / 1_000_000.0), 1.0f, 0.5f, 0.0f);
            return;
        }

        // Skip files larger than 50MB to prevent memory issues
        if (fileSize > 50_000_000) {
            CommandTerminal.printError(String.format("  Skipping large file (%.1f MB): %s", fileSize / 1_000_000.0, filename));
            return;
        }
        ...
        if (nameLower.endsWith(".pdf")) {
            CommandTerminal.printInfo(String.format("[SCRAPER] Extracting PDF: %s (%.1f MB)", filename, fileSize / 1_000_000.0));
            PDDocument doc = null;
            try {
                ...
                if (text.length() > MAX_TEXT_LENGTH) {
                    CommandTerminal.printColored(String.format("  Truncating text from %d to %d chars", text.length(), MAX_TEXT_LENGTH), 1.0f, 0.7f, 0.0f);
